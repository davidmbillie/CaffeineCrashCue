    ...
    stages {
        ...
        stage("Run-Unit-Tests") {
            steps {
                ...
                bat 'dotnet test ./CaffeineCrashProvider.Test/CaffeineCrashCue.Test.csproj -c Release -p:CollectCoverage=true -p:CoverletOutputFormat=cobertura -p:CoverletOutput=./test/coverage/cueCoverage.xml -p:Exclude="[xunit.*]*" -l:"JUnit;LogFilePath=$WORKSPACE/test/results/cueResults.xml" --no-restore --no-build'
            }
        }
	}
	post {
